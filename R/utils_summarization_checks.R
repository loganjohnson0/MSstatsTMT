...
#' @keywords internal
.checkSummarizationParams = function(
  data,
  method,
  global_norm,
  reference_norm,
  ...
  checkmate::assertChoice(
    method,
    c("LogSum", "Median", "MedianPolish", "msstats")
  )
  ...
  if (length(column_diff) > 0) {
    stop(
      paste(
        ...
        paste(column_diff, sep = ", ", collapse = ", "),
        "are missing."
      )
    )
  }
...
#' @keywords internal
.logSummarizationParams = function(
  method,
  global_norm,
  reference_norm,
  remove_norm_channel,
  ...
  msg = paste(
    ...
    paste(
      "Reference-channel based normalization between MS runs :",
      reference_norm
    ),
...
#' @return data.table
#' @keywords internal
.removeRedundantChannels = function(
  input,
  remove_empty_channel,
  remove_norm_channel
) {
