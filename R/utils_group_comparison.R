...
.checkGroupComparisonInput = function(input) {
  ...
  if (!all(required_cols %in% colnames(input))) {
    ...
    missing_msg = paste(required_cols[missing_cols], collapse = ", ")
  ...
  if (data.table::uniqueN(input$Condition) < 2) {
    stop(paste(
      "Please check the Condition column in annotation file.",
...
.checkContrastMatrix = function(contrast_matrix) {
  ...
  if (!(is.matrix(contrast.matrix) | is.data.frame(contrast.matrix))) {
    if (length(contrast.matrix) == 1) {
      # contrast.matrix is a character
    ...
    # contrast.matrix is a matrix or data frame
    contrast.matrix = as.matrix(contrast.matrix)

    if (!all(colnames(contrast.matrix) %in% groups)) {
      stop(
        "Please check the contrast.matrix. Column names of contrast.matrix must be matched with conditions!"
      )
    }

    if (!is.numeric(contrast.matrix)) {
      stop(
        "Please check the contrast.matrix. The elements of the contrast matrix must be all numeric!"
      )
    }
  }
  ...
  if (has_single_subject) {
    ...
    if (has_techreps & has_biomixtures) {
      ...
      if (has_techreps | has_biomixtures) {
        ...
        if (inherits(fit, "try-error")) {
          # run model not applicable - fit one-way anova model
          fit = fit_Group_model(single_protein)
        }
      } else {
        # single run
    ...
    # the data has biological variation
    if (has_Repeated_Measures) {
      # time course design
      if (has_biomixtures) {
        # multiple mixtures
        if (has_techreps) {
          # multiple tech MS runs
          ...
          if (inherits(fit, "try-error")) {
            # full model is not applicable, fit the model with run and subject effects
            fit = fit_Run_Group_Sub_model(single_protein)
          }
          if (inherits(fit, "try-error")) {
            # second model not applicable - fit model with only subject effect
            fit = fit_Group_Sub_model(single_protein)
          }
          if (inherits(fit, "try-error")) {
            # subject model not applicable - fit one-way anova model
            fit = fit_Group_model(single_protein)
          }
        } else {
          # single MS run per mixture
        ...
        if (has_techreps) {
          ...
          if (inherits(fit, "try-error")) {
            # full model not applicable - fit model with only run effect
            fit = fit_Group_Sub_model(single_protein)
          }
          if (inherits(fit, "try-error")) {
            # model not applicable - fit one-way anova model
            fit = fit_Group_model(single_protein)
          }
        } else {
          # single MS run
...
.checkSingleSubject = function(annotation) {
  Subject <- NULL
  count_subjects = annotation[,
    .(NumSubjects = uniqueN(Subject)),
    by = c("Group")
  ]
...
.checkRepeatedMeasures = function(annotation) {
  Group <- NULL
  count_groups = annotation[, .(NumGroups = uniqueN(Group)), by = c("Subject")]
...
fit_Run_Group_model = function(data) {
  suppressMessages(try(
    lmerTest::lmer(Abundance ~ 1 + (1 | Run) + Group, data = data),
    TRUE
  ))
}
...
#' @keywords internal
.handleSingleContrastTMT = function(
  contrast,
  fit,
  single_protein,
  coefs,
...
.handleMissingConditionTMT = function(input, contrast) {
  ...
  if (is.null(positive) | is.null(negative)) {
    stop(
      "Please check the contrast.matrix. 
