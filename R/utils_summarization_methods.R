...
.summarizeMSstats = function(
  ...
  for (i in seq_len(num_runs)) {
    ...
    msg = paste("Summarizing for Run :", runs[i], "(", i, " of ", num_runs, ")")
    getOption("MSstatsTMTLog")("INFO", msg)
    getOption("MSstatsTMTMsg")("INFO", msg)
    ...
    protein_level_data = protein_level_data[, c(
      "Protein",
      "LogIntensities",
      "originalRUN"
    )]
  ...
  summarized_results = summarized_results[,
    colnames(summarized_results) != "RunChannel",
    with = FALSE
  ]
  data.table::setnames(
    processed,
    c("PROTEIN", "PEPTIDE", "originalRUN", "newABUNDANCE"),
    c("ProteinName", "PSM", "RunChannel", "log2Intensity")
  )
  processed = merge(processed, annotation, by = "RunChannel", all.x = TRUE)
  processed[,
    c("PeptideSequence", "Charge") := tstrsplit(PSM, "_", fixed = TRUE)
  ]
...
.summarizeTMP = function(input, annotation) {
  ...
  new_annotation = unique(input[, list(
    Run,
    ProteinName,
    Channel,
  ...
  summarized = input[,
    list(MedianPolish = .medianPolish(log2Intensity, channel_len)),
