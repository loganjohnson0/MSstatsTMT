...
groupComparisonTMT = function(
  ...
  getOption("MSstatsTMTLog")(
    "INFO",
    paste("Remove empty channels :", remove_empty_channel)
  )
  getOption("MSstatsTMTLog")(
    "INFO",
    paste("Remove normalization channels :", remove_norm_channel)
  )

  summarized = MSstatsPrepareForGroupComparisonTMT(
  ...
  testing_results = MSstatsGroupComparisonOutputTMT(testing_results, adj.method)

  if (save_fitted_models) {
    list(
      ComparisonResult = testing_results,
      ModelQC = NULL,
      FittedModel = FittedModel
    )
  } else {
    list(ComparisonResult = testing_results, ModelQC = NULL, FittedModel = NULL)
  }
}
...
MSstatsFitComparisonModelsTMT = function(input) {
  ...
  if (has_techreps) {
    msg = paste0(
      "Design: ",
      length(unique(annotation$TechRepMixture)),
  ...
  if (has_single_subject) {
    ...
    if (has_Repeated_Measures) {
      msg = paste0(
        "Design: repeated measures design (A biological subject is measured in multiple conditions)."
      )
    } else {
      msg = paste0(
        "Design: group comparison design (Different conditions contains different biological subjects)."
      )
    }
    getOption("MSstatsTMTLog")("INFO", msg)
    getOption("MSstatsTMTMsg")("INFO", msg)
  ...
  for (i in seq_along(all_proteins)) {
    ...
    model_fit_result = MSstatsComparisonModelSingleTMT(single_protein, protein)
...
MSstatsComparisonModelSingleTMT = function(single_protein, protein_name) {
  ...
  annotation = unique(single_protein[, list(
    Run,
    Channel,
    Subject,
    Group,
...
MSstatsModerateTTest = function(summarized, fitted_models, moderated) {
  ...
  eb_input_s2 = fitted_models[variance_df != 0 & !is.na(variance_df), variance]
  eb_input_df = fitted_models[
    variance_df != 0 & !is.na(variance_df),
    variance_df
  ]
  if (moderated) {
...
MSstatsGroupComparisonTMT = function(fitted_models, contrast_matrix) {
  ...
  for (i in seq_along(fitted_models)) {
    testing_result = MSstatsTestSingleProteinTMT(
      fitted_models[[i]],
      contrast_matrix
    )
  ...
  if (!inherits(fit, "try-error")) {
    ...
    for (row_id in seq_len(nrow(contrast_matrix))) {
      ...
      if (
        ...
        FC = (cm %*% coefs)[1, 1]
        if (inherits(fit, "lm")) {
          se2.post = diag(t(cm) %*% summary(fit)$cov.unscaled %*% cm) *
            s2_posterior
...
MSstatsGroupComparisonOutputTMT = function(testing_results, adj_method) {
  ...
  testing_results[,
    adj.pvalue := p.adjust(pvalue, adj_method),
    by = "Comparison"
  ]
