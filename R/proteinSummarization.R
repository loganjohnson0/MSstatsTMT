#' Summarize into proteins
#'
#' We assume missing values are censored and then impute the missing values.
#' Protein-level summarization from peptide-level quantification is performed.
#' Subsequently, global median normalization on peptide-level data and
...
#'   Run, Channel, Condition, BioReplicate, and Intensity.
...
#' @param MBimpute Only for method = "msstats".
#'   * TRUE (default): imputes missing values by Accelated failure model.
#'   * FALSE: imputes the minimum value for each peptide precursor ion.
...
#'   missing value, for instance, 0.999. Default is NULL.
#' @param remove_norm_channel TRUE (default) removes 'Norm' channels from
#'   protein-level data.
#' @param remove_empty_channel TRUE (default) removes 'Empty'
...
proteinSummarization = function(
  ...
  summarized = MSstatsSummarizeTMT(
    input,
    method,
    MBimpute,
    maxQuantileforCensored,
  ...
  output = MSstatsSummarizationOutputTMT(
    summarized,
    processed,
    remove_empty_channel,
    remove_norm_channel
...
MSstatsPrepareForSummarizationTMT = function(
  ...
  options(MSstatsLog = current_msstats_log, MSstatsMsg = current_msstats_msg)
...
#' @keywords internal
#'
MSstatsSummarizationOutputTMT = function(
  summarized,
  processed,
  remove_empty_channel,
  remove_norm_channel
) {
  summarized = .removeRedundantChannels(
    summarized,
    remove_empty_channel,
    remove_norm_channel
  )
  list(
    FeatureLevelData = as.data.frame(processed),
