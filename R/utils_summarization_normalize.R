...
MSstatsNormalizeTMT = function(input, type, normalize) {
  ...
  if (type == "peptides") {
    ...
    if (any(input$Intensity < 1 & !is.na(input$Intensity))) {
      input[,
        log2Intensity := ifelse(
          Intensity < 1 & !is.na(Intensity),
          NA,
          log2Intensity
        )
      ]
      input[,
        Intensity := ifelse(Intensity < 1 & !is.na(Intensity), NA, Intensity)
      ]
    }
  } else {
...
.normalizePeptides = function(input, normalize) {
  ...
  if (normalize) {
    input[,
      MedianLog2Int := median(log2Intensity, na.rm = TRUE),
      by = c("Run", "Channel")
    ]
...
.normalizeProteins = function(input, normalize) {
  ...
  if (normalize & (n_runs > 1)) {
    ...
    if (is.element("Norm", group_info)) {
      ...
      input[
        !is.na(Abundance),
        NormalizationAbundance := .getNormalizationAbundance(
          Abundance,
          Condition
        ),
        by = c("Protein", "Run")
      ...
      input[
        ...
        by = "Protein",
        .SDcols = c("Run", "NormalizationAbundance")
      ]
      input[
        !is.na(Abundance),
        Diff := MedianNormalized - NormalizationAbundance
      ]
      input[!is.na(Abundance), NormalizedAbundance := Abundance + Diff]
      input[,
        Abundance := ifelse(
          NumRuns > 1 & NumRunsWithNorm > 1,
          NormalizedAbundance,
          Abundance
        )
      ]
