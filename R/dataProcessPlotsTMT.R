...
dataProcessPlotsTMT = function(
  ...
  checkmate::assertChoice(
    toupper(type),
    c("PROFILEPLOT", "QCPLOT"),
    .var.name = "type"
  )
  if (address == FALSE) {
    if (which.Protein == "all") {
      stop(paste(
        "** Cannnot generate all plots in a screen.",
        "Please set one protein at a time."
      ))
    } else if (length(which.Protein) > 1) {
      stop(paste(
        "** Cannnot generate multiple plots in a screen.",
        "Please set one protein at a time."
      ))
    }
  }
  warning(
    "Avoid plotting all proteins as it can take a large amount of time 
            to download the files"
  )
  if (isPlotly & address != FALSE) {
    print(
      "Plots will be saved as .HTML file as plotly is selected, set isPlotly = FALSE, if 
            you want to generate PDF using ggplot2"
    )
  }

  if (toupper(type) == "PROFILEPLOT") {
    plots <- .plotProfileTMT(
      processed,
      summarized,
      featureName,
      ylimUp,
    ...
    if (isPlotly) {
      ...
      if ("original_plot" %in% names(plots)) {
        for (i in seq_along(plots[["original_plot"]])) {
          ...
          og_plotly_plot <- .convertGgplot2Plotly(
            plot_i,
            tips = c("PSM", "xorder", "abundance", "censored")
          )
          og_plotly_plot = .fixLegendPlotlyPlotsDataprocess(
            og_plotly_plot,
            "OriginalPlot"
          )
      ...
      if ("summary_plot" %in% names(plots)) {
        for (i in seq_along(plots[["summary_plot"]])) {
          ...
          summ_plotly_plot = .fixLegendPlotlyPlotsDataprocess(
            summ_plotly_plot,
            "SummaryPlot"
          )
          if (toupper(featureName) == "NA") {
...
.plotProfileTMT = function(
  ...
  if (which.Protein != "all") {
    chosen_proteins = getSelectedProteins(
      which.Protein,
      unique(processed$Protein)
    )
  ...
  if (originalPlot) {
    ...
    for (i in seq_along(all_proteins)) {
      ...
      single_protein$PeptideSequence = factor(as.character(
        single_protein$PeptideSequence
      ))
      ...
      if (
        all(is.na(single_protein$abundance)) |
      ...
      ptemp = ggplot(
        ...
        labs(title = unique(single_protein$Protein), x = 'MS runs') +
        ...
        guides(
          color = guide_legend(
            title = paste(
              "# peptide:",
              nlevels(single_protein$PeptideSequence)
            ),
  ...
  if (summaryPlot) {
    ...
    message(paste0(
      "Drew the Profile plot with summarization for ",
      length(all_proteins),
      " proteins."
    ))
    pb = txtProgressBar(max = length(all_proteins), style = 3)
    for (i in seq_along(all_proteins)) {
      ...
      final = rbind(
        single_protein[, colnames(quantrun), with = FALSE],
        quantrun
      )
      ...
      ptempall = ggplot(
        ...
        scale_linetype_manual(
          values = c(rep(1, times = length(unique(final$PSM)) - 1), 2),
          guide = "none"
        ) +
        labs(title = unique(single_protein$Protein), x = 'MS runs') +
        scale_y_continuous(yaxis.name, limits = c(y.limdown, y.limup)) +
      ...
      ptempall = ptempall +
        geom_point(
          data = final,
          aes(x = xorder, y = abundance, size = analysis, color = analysis)
        )
      print(ptempall)
...
.plotQualityTMT = function(
  ...
  processed = .getXAxisOrder(processed)
  tempGroupName = unique(processed[, c(
    "Condition",
    "xorder",
    "Run",
    "Channel"
  ...
  if (which.Protein == "all" | which.Protein == "allonly") {
    ...
    ptemp = ggplot(
      ...
      facet_grid(~Run) +
      geom_boxplot(
        aes_string(fill = "Condition"),
        outlier.shape = 1,
        outlier.size = 1.5
      ) +
      labs(title = "All proteins", x = "MS runs") +
      scale_y_continuous(yaxis.name, limits = c(y.limdown, y.limup)) +
      geom_vline(
        data = groupline.tmp,
        aes(xintercept = cumGroupAxis),
      ...
      theme_msstats("QCPLOT", x.axis.size, y.axis.size, legend_size = NULL) +
      theme(
        axis.ticks.x = element_blank(),
        axis.text.x = element_blank(),
        strip.text.x = element_text(
  ...
  if (which.Protein != "allonly") {
    ...
    all_proteins = unique(processed$Protein)
    message(paste0(
      "Drew the Quality Contol plot(boxplot) for each of ",
      length(all_proteins),
      " proteins."
    ))
    pb = txtProgressBar(max = length(all_proteins), style = 3)
...
facet_strip_bigger <- function(gp) {
  if (!is.null(gp$x$layout$annotations)) {
    for (i in seq_along(gp$x$layout$annotations)) {
      if (
        gp$x$layout$annotations[[i]]$text != "Log2-intensities" &&
          gp$x$layout$annotations[[i]]$text != "MS runs"
      ) {
...
.getPlotlyPlotHTML = function(plots, width, height) {
  doc <- tagList(lapply(
    plots,
    function(x) div(x, style = "float:left;width:100%;")
  ))
...
.fixLegendPlotlyPlotsDataprocess = function(plot, type) {
  df <- data.frame(
    id = seq_along(plot$x$data),
    legend_entries = unlist(lapply(plot$x$data, `[[`, "name"))
  )
  ...
  df$is_valid_column <- ifelse(
    grepl("Processed feature-level data|Run summary", df$legend_entries),
    TRUE,
    FALSE
  )
...
getFileName = function(name_base, file_name, width, height) {
  ...
  if (file_name == 'ProfilePlot') {
    num_same_name = sum(grepl(
      paste0("^", name_base, file_name, "_[0-9]?"),
      all_files
    ))
  } else {
    num_same_name = sum(grepl(
      paste0("^", name_base, file_name, "[0-9]?"),
      all_files
    ))
  }
  if (num_same_name > 0) {
...
.getGroupLabel = function(input, y.limup) {
  ...
  input[,
    Condition := factor(Condition, levels = unique(Condition), ordered = TRUE)
  ]
