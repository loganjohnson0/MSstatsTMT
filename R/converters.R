...
#' @param use_log_file logical. If TRUE, information about data processing will be saved to a file.
#' @param append logical. If TRUE, information about data processing will be added to an existing log file.
#' @param verbose logical. If TRUE, information about data processing wil be printed to the console.
#' @param log_file_path character. Path to a file to which information about data processing will be saved. If not provided, such a file will be created automatically. If `append = TRUE`, has to be a valid path to a file.
#'
#' @return NULL.
#' @keywords internal
...
MaxQtoMSstatsTMTFormat = function(
  ...
  input = MSstatsConvert::MSstatsImport(
    list(evidence = evidence, protein_groups = proteinGroups),
    "MSstatsTMT",
    "MaxQuant",
    ...
  )
  ...
  input = MSstatsConvert::MSstatsPreprocess(
    ...
    feature_cleaning = list(
      remove_features_with_few_measurements = rmPSM_withfewMea_withinRun,
  ...
  msg_final = paste(
    "** Finished preprocessing. The dataset is ready",
...
PhilosophertoMSstatsTMTFormat = function(
  ...
  input = MSstatsConvert::MSstatsImport(
    c(mixture_files, list(annotation = annotation)),
    type = "MSstatsTMT",
    tool = "Philosopher"
  )
  ...
  probability_filter = list(
    score_column = "PeptideProphetProbability",
  ...
  oxidation_filter = list(
    col_name = "PeptideSequence",
    pattern = "Oxidation",
...
.getPhilosopherInput = function(input, path, folder) {
  if (!is.null(input)) {
    ...
    if (folder) {
      mixture_files = list.files(path, pattern = "msstats", full.names = TRUE)
    } else {
      mixture_files = path
    }
  }
